...
int main(int argc,char*argv[])
{
    ...
    strcpy(sql, "select ciphertext from user where id = 1");
    ...
    bzero(resBuf, BUFFER_SIZE);
    strcpy(sql, "select ciphertext from user where id = 2");
    ret = execute_sql(db, sql);
...
char *get_result_to_string(MYSQL *mysql, char *resBuf)
{
    ...
    if (rows != 0)
    {
        ...
        printf("resBuf = %s\n", resBuf);
        if ( strncmp(resBuf, "(null)", 6) == 0 )
        {
            puts("------------succeded--------------");
        }
        //if ( strcmp(resBuf, "(null) ") == 0 )//经测试貌似并不是空格，只是打印出来是空格
        //{
        //    puts("-----------------succeeded-----------------");
        //}
    }
